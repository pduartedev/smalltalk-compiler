...
Expressao *Expressao::extrai_expressao(No_arv_parse *no)
{
  ...
  // Keyword_Argument -> Primary | Primary Binary_Message_List
  if (no->simb == "Keyword_Argument")
  {
    if (no->filhos.size() >= 2)
    {
      // Primary com Binary_Message_List
      Expressao *primary = extrai_expressao(no->filhos[0]);
      if (primary != nullptr && no->filhos[1]->simb == "Binary_Message_List")
      {
        return extrai_binary_message_list(primary, no->filhos[1]);
      }
      return primary;
    }
    else if (no->filhos.size() >= 1)
